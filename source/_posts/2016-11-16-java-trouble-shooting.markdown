...
jcmd提供了输出HPROF格式的堆dump接口。运行**jcmd <PID> GC.heap_dump <FILENAME>**即可。注意这里的FILENAME是相对于运行中的jvm目录来说的，因此避免找不到dump的文件，这里推荐使用绝对路径。此外，也建议使用.hprof作为输出文件的扩展名。
