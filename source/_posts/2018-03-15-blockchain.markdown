...
区块体的信息主要是交易列表。交易的数据结构因平台的不同而不同。比特币中就是简单的交易信息，以太坊则会存放智能合约。这里需要提到的是比特币中使用UTXO（未花费的交易输出，Unspent Transaction Output）实现支付系统的账户模型，并没有余额、钱包的概念（比特币=UTXO），交易列表中的关键信息也是一些UTXO的组成。如此计算账户的余额则需要通过遍历整个交易历史来最终计算出每个账户的余额，能够避开基于账户概念的设计的一些缺陷。而以太坊则使用了传统所理解的账户模型。UTXO的架构如下图所示：
